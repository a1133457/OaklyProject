...
const { onAdd, openSuccessModal } = useCart;
    ...
    const fetchSimilarProducts = async () => {
      try {
        ...
        if (Array.isArray(allProducts)) {
          ...
          const randomProducts = shuffled.slice(
            0,
              ...
              <div className="product-image">
                ...
                <button
                  ...
                  onClick={(e) => {
                    ...
                    addToCartFromModal && addToCartFromModal(product);
                  }}
                >
                  <div className="product-actions">加入購物車</div>
                </button>
              </div>
                ...
                <div className="product-info">
                  <h3 className="product-name">
                    {failedImages.has(product.id)
                      ? "nana椅"
                      : product.name || "商品名稱載入中..."}
                  </h3>
                  <p className="product-price">
                    NT${" "}
                    {failedImages.has(product.id)
                      ? "2500"
                      : product.price?.toLocaleString() || "價格確認中"}
                  </p>
                </div>
              </div>
            </div>
