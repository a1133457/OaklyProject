...
const MainProduct = () => {
  ...
  const addToCartFromModal = () => {
    ...
    onAdd(currentCartProduct, cartQuantity, selectedColor, selectedSize);
  ...
  const removeFromWishlist = async (productId) => {
    try {
      const userId = localStorage.getItem("userId") || 1;
      const response = await fetch(
        `http://localhost:3005/api/wishlist/${userId}/${productId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
                ...
                {currentProducts.map((product) => (
                  ...
                  >
                    ...
                    >
                      <i
                        className={`fa-${
                          isWishlisted[product.id] ? "solid" : "regular"
                        } fa-heart`}
                      ></i>
                    </button>
                  </div>
